...
const Register = () => {
    ...
    const handleRegister=async(e)=>{
        e.preventDefault();
        try{
            await axios.post('http://localhost:3333/user/register',{username,email,password});
            navigate('/login')
        }catch (err) {
  if (err.response) {
    alert(err.response.data.error); // Show specific backend error
  } else {
  ...
  return (
    ...
    <form onSubmit={handleRegister} className='w-120 p-6 bg-gray-900 rounded-md shadow-md flex flex-col gap-4'>
        ...
        <input type="password" placeholder='Enter Your Password' onChange={e=>setPassword(e.target.value)}
         className='h-13 p-2 rounded bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500' required/>
        <button type='submit'   className='h-13 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded font-semibold'>Register</button>
